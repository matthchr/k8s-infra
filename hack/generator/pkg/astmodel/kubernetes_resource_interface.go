...
func AddKubernetesResourceInterfaceImpls(
	...
	// Add defaults
	defaulterBuilder := NewDefaulterBuilder(resourceName, spec, idFactory)
	if setNameFunction != nil {
		defaulterBuilder.AddDefault(NewDefaultAzureNameFunction(spec, idFactory))
	}
	r = r.WithInterface(defaulterBuilder.ToInterfaceImplementation())

	// Add validations
	validatorBuilder := NewValidatorBuilder(resourceName, spec, idFactory)
	validatorBuilder.AddValidation(ValidationKindCreate, NewValidateResourceReferencesFunction(spec, idFactory))
	validatorBuilder.AddValidation(ValidationKindUpdate, NewValidateResourceReferencesFunction(spec, idFactory))
