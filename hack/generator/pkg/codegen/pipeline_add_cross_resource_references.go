...
var armIDDescriptionRegex = regexp.MustCompile(".*/subscriptions/.*?/resourceGroups/.*")
...
func makeCrossResourceReferenceTypeVisitor(idFactory astmodel.IdentifierFactory, referenceChecker crossResourceReferenceChecker) crossResourceReferenceTypeVisitor {
	...
	transformResourceReferenceProperties := func(this *astmodel.TypeVisitor, it *astmodel.ObjectType, ctx interface{}) (astmodel.Type, error) {
...
func makeResourceReferenceProperty(idFactory astmodel.IdentifierFactory, existing *astmodel.PropertyDefinition) *astmodel.PropertyDefinition {
	...
	// Special case for "Id" which is quite common in the specs and also contains no usage information we're afraid to lose (as opposed to
	// something like "vnet" what says what the reference is pointing to and we don't want to overwrite to something generic).
